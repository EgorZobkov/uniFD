...
class AdsFilters extends Model
{

    public $alias = 'ads_filters';
    public $table = 'uni_ads_filters';
    ...
    public function getAll($conditions=null, $params=[]){
        ...
        if(isset($this->filter)){
            ...
            if(isset($this->filter["category_id"])){
                ...
                $getFilterIds = $app->component->ads_filters->getFiltersByCategory($this->filter["category_id"]);

                if($getFilterIds){
                    $query[] = 'id IN('.implode(",", $getFilterIds).')';
                }else{
                    $query[] = 'id IN(0)';
                }
