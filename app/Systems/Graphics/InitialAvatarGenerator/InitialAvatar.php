...
namespace App\Systems\Graphics\InitialAvatarGenerator;

use Intervention\Image\ImageManager;
use Intervention\Image\Drivers\Gd\Driver;
use App\Systems\Graphics\InitialAvatarGenerator\Translator\Base;
use App\Systems\Graphics\InitialAvatarGenerator\Translator\En;
use App\Systems\Graphics\InitialAvatarGenerator\Translator\Tr;
use App\Systems\Graphics\InitialAvatarGenerator\Initials;
use App\Systems\Graphics\InitialAvatarGenerator\StringScript;
...
class InitialAvatar
{
    ...
    protected $generated_initials = 'JO';
    ...
    {
        $this->image = new ImageManager(Driver::class);
    }
    ...
    protected function makeAvatar($image)
    {
        ...
        $avatar = $image->create($width, $height)->fill(!$this->getRounded() ? $bgColor : null);

        if ($this->getRounded()) {
            $avatar = $avatar->drawCircle($width - 2, $width / 2, $height / 2, function ($draw) use ($bgColor) {
